...
const ids = ['salaire', 'loyer', 'nourriture', 'assurance', 'dette', 'epargne'];
...
function compute() {
  ...
  const totalCharges = loyer + nourriture + assurance + dette;
  const resteAVivre  = salaire - totalCharges - epargne;
  const tauxEpargne  = salaire > 0 ? (epargne / salaire) * 100 : 0;

  return { salaire, loyer, nourriture, assurance, dette, epargne, totalCharges, resteAVivre, tauxEpargne };
}

// ── Mise à jour DOM ────────────────────────────────────────────
function updateDOM(data) {
  const { totalCharges, resteAVivre, tauxEpargne, epargne, salaire } = data;
...
function updateChart(data) {
  const { loyer, nourriture, assurance, dette, epargne, resteAVivre } = data;

  const restePositif = Math.max(0, resteAVivre);
  const labels  = ['Loyer', 'Nourriture', 'Assurance', 'Dette', 'Épargne', 'Reste à vivre'];
  const values  = [loyer, nourriture, assurance, dette, epargne, restePositif];
  const colors  = ['#4f46e5', '#f59e0b', '#10b981', '#ef4444', '#06b6d4', '#a3e635'];
